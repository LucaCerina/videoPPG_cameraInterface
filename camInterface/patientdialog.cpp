...
void PatientDialog::on_buttonBox_accepted()
{
	...
	if(checkPtData())
	{
		msgBox = QMessageBox::question(this, "Continue", "Are you sure?",
	...
	{
		QMessageBox::warning(this, "Error in data",
							 "Missing or invalid data, check again.");
		this->show();
	}
...
void PatientDialog::on_IdEdit_editingFinished()
{
	...
	if(QDir(ptPath).exists() && ui->IdEdit->text() != "")
	{
		QMessageBox::warning(this,"Id not available",
							 "Id already used.Change the patient Id");
		ui->IdEdit->setText("");
		this->show();
	}
}
